...
select substring(first_name, 1, 3) as Name_initials from worker;
...
select w.* from worker as w join title as t on w.worker_id = t.worker_ref_id where t.worker_title = 'manager'; -- better practice
...
insert into newTable select * from worker;
...
-- DUAL
select curdate(); -- gives only date
